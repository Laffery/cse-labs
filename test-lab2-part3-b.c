...
create1(const char *d, const char *f, const char *in)
{
  ...
  fd = creat(n, 0666);
  if(fd < 0){
    fprintf(stderr, "test-lab2-part3-b: create(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  if(write(fd, in, strlen(in)) != strlen(in)){
    fprintf(stderr, "test-lab2-part3-b: write(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  if(close(fd) != 0){
    fprintf(stderr, "test-lab2-part3-b: close(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
}
...
check1(const char *d, const char *f, const char *in)
{
  ...
  fd = open(n, 0);
  if(fd < 0){
    fprintf(stderr, "test-lab2-part3-b: open(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  errno = 0;
  ...
  if(cc != strlen(in)){
    fprintf(stderr, "test-lab2-part3-b: read(%s) returned too little %d%s%s\n",
            n,
            cc,
            errno ? ": " : "",
  ...
  if(strncmp(buf, in, strlen(n)) != 0){
    fprintf(stderr, "test-lab2-part3-b: read(%s) got \"%s\", not \"%s\"\n",
            n, buf, in);
    exit(1);
  }
}
...
unlink1(const char *d, const char *f)
{
  ...
  sprintf(n, "%s/%s", d, f);
  if(unlink(n) != 0){
    fprintf(stderr, "test-lab2-part3-b: unlink(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
}
...
checknot(const char *d, const char *f)
{
  ...
  fd = open(n, 0);
  if(fd >= 0){
    fprintf(stderr, "test-lab2-part3-b: open(%s) succeeded for deleted file\n", n);
    exit(1);
  }
}
...
append1(const char *d, const char *f, const char *in)
{
  ...
  if(fd < 0){
    fprintf(stderr, "test-lab2-part3-b: append open(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  if(write(fd, in, strlen(in)) != strlen(in)){
    fprintf(stderr, "test-lab2-part3-b: append write(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  if(close(fd) != 0){
    fprintf(stderr, "test-lab2-part3-b: append close(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
}
...
checkn(const char *d, const char *prefix, int nf)
{
  ...
  for(i = 0; i < nf; i++){
    ...
    if(j != i){
      fprintf(stderr, "test-lab2-part3-b: checkn %s contained %d not %d\n",
              n, j, i);
      exit(1);
    }
    close(fd);
...
dircheck(const char *d, int nf)
{
  ...
  dp = opendir(d);
  if(dp == 0){
    fprintf(stderr, "test-lab2-part3-b: opendir(%s): %s\n", d, strerror(errno));
    exit(1);
  }
  while((e = readdir(dp))){
    if(e->d_name[0] != '.'){
  ...
  if(nf != nnames){
    fprintf(stderr, "test-lab2-part3-b: wanted %d dir entries, got %d\n", nf, nnames);
    exit(1);
  }
  ...
  for(i = 0; i < nnames-1; i++){
    if(strcmp(names[i], names[i+1]) == 0){
      fprintf(stderr, "test-lab2-part3-b: duplicate directory entry for %s\n", names[i]);
      exit(1);
    }
  }
...
main(int argc, char *argv[])
{
  ...
  if(argc != 3){
    fprintf(stderr, "Usage: test-lab2-part3-b dir1 dir2\n");
    exit(1);
  }

  sprintf(d1, "%s/da%d", argv[1], getpid());
  if(mkdir(d1, 0777) != 0){
    fprintf(stderr, "test-lab2-part3-b: failed: mkdir(%s): %s\n",
            d1, strerror(errno));
    exit(1);
  }
...

  {
    ...
    if(access(dd, 0) != 0){
      fprintf(stderr, "test-lab2-part3-b: failed: access(%s) after mkdir %s: %s\n",
